...
void main() {
  ...
  group('OfflinePronunciationAnalyzer', () {
    test('engine id is offline-local', () async {
      final result = await analyzer.analyze('hello world', 'hello world');
      expect(result.engine, equals('offline-local'));
    });

    test(
      'identical inputs produce identical scores (determinism / AC #6)',
      () async {
    ...
    test(
      'empty spoken text returns a zero-score result, not a throw (AC #3)',
      () async {
    ...
    test(
      'perfect match scores 100 for completeness; overall is the per-word average',
      () async {
        final result = await analyzer.analyze('one two three', 'one two three');
    ...
    test('partial match scores partial completeness', () async {
      ...
      final result = await analyzer.analyze('one two three', 'one');
