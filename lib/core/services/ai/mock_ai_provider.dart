  ...
  Future<AiGrammarAnalysis> analyzeGrammar(String text) async {
    ...
    if (lowerText.contains('i is') || lowerText.contains('i are')) {
      return AiGrammarAnalysis(
        ...
        correctedSentence: trimmedText.replaceAll(RegExp('i is', caseSensitive: false), 'I am').replaceAll(RegExp('i are', caseSensitive: false), 'I am'),
        explanation: 'In English, the first-person singular pronoun "I" always takes the verb form "am" in the present tense, not "is" or "are".',
        grammarScore: 75,
      );
    }

    if (lowerText.contains('he go ') || lowerText.contains('she go ') || lowerText.contains('it go ')) {
      return AiGrammarAnalysis(
        originalSentence: trimmedText,
        correctedSentence: trimmedText.replaceAll(RegExp(' go ', caseSensitive: false), ' goes '),
        explanation: 'Third-person singular subjects (he, she, it) require the verb to have an "-es" or "-s" ending in the simple present tense (e.g., "goes").',
        grammarScore: 80,
      );
    }
    ...
    if (lowerText.contains('she don\'t') || lowerText.contains('he don\'t')) {
      return AiGrammarAnalysis(
        originalSentence: trimmedText,
        correctedSentence: trimmedText.replaceAll(RegExp('don\'t', caseSensitive: false), 'doesn\'t'),
        explanation: 'Use "doesn\'t" (does not) for third-person singular subjects (he, she, it). "Don\'t" is used for I, you, we, and they.',
        grammarScore: 85,
      );
    }
    ...
    return AiGrammarAnalysis(
      ...
      explanation: 'Your sentence is grammatically correct! Good job structuring the sentence.',
      grammarScore: 100,
    );
  }
  ...
  Future<List<AiVocabularyWord>> suggestVocabulary(String topic, {String difficulty = 'Intermediate'}) async {
    ...
    if (lowerTopic.contains('travel')) {
      return [
        AiVocabularyWord(
          word: 'Itinerary',
          definition: 'A planned route or journey, including dates and places to visit.',
          example: 'We formulated a strict itinerary for our two-week vacation in Europe.',
          difficulty: difficulty,
        ),
        AiVocabularyWord(
          word: 'Wanderlust',
          definition: 'A strong desire to travel and explore the world.',
          example: 'Her wanderlust led her to abandon her desk job and travel Asia.',
          difficulty: difficulty,
        ),
        AiVocabularyWord(
          word: 'Souvenir',
      ...
      return [
        AiVocabularyWord(
          word: 'Gourmet',
          definition: 'High-quality or exotic food, or a person who appreciates fine dining.',
          example: 'He opened a gourmet restaurant serving locally sourced ingredients.',
          difficulty: difficulty,
        ),
        AiVocabularyWord(
          word: 'Cuisine',
          definition: 'A style or method of cooking, especially as characteristic of a particular country or region.',
          example: 'Italian cuisine is famous for its simple but delicious pasta dishes.',
          difficulty: difficulty,
        ),
        AiVocabularyWord(
          word: 'Delectable',
    ...
    return [
      AiVocabularyWord(
        ...
        example: 'It takes time to acquire a native-like accent in a foreign tongue.',
        difficulty: difficulty,
      ),
      AiVocabularyWord(
        word: 'Fluency',
        definition: 'The ability to express oneself easily and articulately.',
        example: 'Practice speaking daily is the fastest way to achieve fluency.',
        difficulty: difficulty,
      ),
      AiVocabularyWord(
        word: 'Accentuate',
        definition: 'Make more noticeable or prominent.',
        example: 'Try not to accentuate your errors; focus on speaking smoothly.',
        difficulty: difficulty,
      ),
    ];
  }
